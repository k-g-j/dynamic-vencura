...
export class AuthService {
  ...
  async verifyDynamicToken(token: string): Promise<DynamicUser | null> {
    try {
      ...
      console.log('Verifying Dynamic token with environment ID:', env.DYNAMIC_ENVIRONMENT_ID);
      ...
      return new Promise((resolve) => {
        jwt.verify(token, getKey, { algorithms: ['RS256'] }, (err, decoded) => {
          if (err) {
            ...
            console.error('JWKS URI:', `https://app.dynamic.xyz/api/v0/sdk/${env.DYNAMIC_ENVIRONMENT_ID}/.well-known/jwks`);
            resolve(null);
            return;
          }
